...
public interface TransactionRepository
    ...
    @Query("""
            SELECT new com.zorvyn.financedashboard.dto.response.CategorySummaryResponse(
                t.category,
                t.type,
                SUM(t.amount),
                COUNT(t)
    ...
    @Query("""
            SELECT
                ...
                SUM(CASE WHEN t.type = com.zorvyn.financedashboard.enums.TransactionType.INCOME
                         THEN t.amount ELSE 0 END),
                SUM(CASE WHEN t.type = com.zorvyn.financedashboard.enums.TransactionType.EXPENSE
                         THEN t.amount ELSE 0 END)
            FROM Transaction t
            WHERE t.isDeleted = false
            GROUP BY
